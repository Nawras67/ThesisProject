...
#  error "Either Mongoose or Civetweb must be enabled to compile this file"
  ...
  
    {
      ...
      // HTTP server
      int port = lock.GetConfiguration().GetUnsignedIntegerParameter("HttpPort", 5000);
      std::cout << "Hello, Mark on Main 1032"<< port;
      httpServer.SetThreadsCount(lock.GetConfiguration().GetUnsignedIntegerParameter("HttpThreadsCount", 50));
      httpServer.SetPortNumber(lock.GetConfiguration().GetUnsignedIntegerParameter("HttpPort", 8040));
      httpServer.SetRemoteAccessAllowed(lock.GetConfiguration().GetBooleanParameter("RemoteAccessAllowed", true));
                             ...
                             OrthancPlugins* plugins)
{
  ...
  else
  {
...

    {
      ...
      dicomServer.SetPortNumber(lock.GetConfiguration().GetUnsignedIntegerParameter("DicomPort", 104));
