...
namespace Orthanc
{
  ...
  {
    bool test = json_.isMember(parameter);
  //  std::cout<<"\nParameter: " << parameter << "\n IsMember :"<< test <<"\n"; 
  ...
  unsigned int OrthancConfiguration::GetUnsignedIntegerParameter(const std::string& parameter,unsigned int defaultValue)
  ...
  bool OrthancConfiguration::SetupRegisteredUsers(HttpServer& httpServer) const
  {
   // std::cout <<"\n Check Registered Users\n";
    ...
    {
    //  std::cout <<"\nRegistered Users've not been readen\n";
      return false;
    }
  ...
  std::string OrthancConfiguration::GetDatabaseServerIdentifier() const
  {
    ...
    else
    {
      ...
      items.insert("dicom-port=" + boost::lexical_cast<std::string>(GetUnsignedIntegerParameter("DicomPort", 104)));
      items.insert("http-port=" + boost::lexical_cast<std::string>(GetUnsignedIntegerParameter("HttpPort", 8040)));
